...
public class FatTree extends Topology{
    ...
    public void addFlow(int s,int d, int flowid, int packets,int w){
        ...
        if(h1 == h2){
            if(h3 == h4){
                network.addFlow(flowid,packets,w,new int[]{podes[h2].getEdge(h4)});
            }
            else{
                int randAggr = h5;
                network.addFlow(flowid,packets,w,new int[]{podes[h1].getEdge(h3),podes[h2].getAggr(randAggr),podes[h2].getEdge(h4)});
            }
        }
        else{
                int randAggr = h5;
