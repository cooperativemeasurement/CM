...
class Geant extends Topology{
    ...
    void printShortestDistance( int src, int dest,int flowId,int packetId,int w)
    { 
        int[] pred = new int[v];
        int[] dist = new int[v]; 
        ...
        network.addFlow( flowId, packetId, w,p);
    } 



    public void addFlow(int s,int d, int flowid, int packets,int w){
        int src = h1.hash(s,v);
        int dist = h1.hash(d,v);
        if(src == dist){
            int[] p = new int[1];
            p[0]=src;
            network.addFlow(flowid, packets, w,p);
        }
        else{
            printShortestDistance(src, dist,flowid, packets,w);
        }
